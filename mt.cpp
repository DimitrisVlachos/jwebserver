...
bool mt_init(const uint32_t thread_count) {
	...
	g_thread_ctx_heap_entry = (uint8_t*)malloc(cnt * (sizeof(ptrdiff_t**) + sizeof(pthread_t) + sizeof(uint32_t)));	
	...
	const pthread_mutex_t cp = PTHREAD_MUTEX_INITIALIZER;
	g_mutex = cp;
	return true;
}
...
void mt_shutdown() {
	if (g_thread_ctx_heap_entry)
		free(g_thread_ctx_heap_entry);
